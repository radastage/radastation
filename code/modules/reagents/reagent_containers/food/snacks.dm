...
/obj/item/weapon/reagent_containers/food/snacks/monkeycube
	...
	afterattack(obj/O, mob/user)
		...
		if(istype(O,/obj/item/weapon/reagent_containers) && !wrapped && O.reagents.has_reagent("water"))
			user << "<span class='notice'>You place [src] in [O].</span>"
			user.drop_item()
			loc = get_turf(O)
			return Expand()
		..()

		if(istype(O,/obj/structure/reagent_dispensers/watertank) && !wrapped && O.reagents.has_reagent("water"))
			user << "<span class='notice'>You place [src] in [O].</span>"
			user.drop_item()
			loc = get_turf(O)
			return Expand()
