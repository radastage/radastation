...
/mob/living/carbon/monkey/update_inv_wear_mask(var/update_icons=1)
	if(wear_mask && istype(wear_mask, /obj/item/clothing/mask) )
		...
		var/icon/mask_l = icon("icon"='icons/mob/monkey.dmi', "icon_state"="[wear_mask.icon_state]2")
		var/icon/mask_s = icon("icon"='icons/mob/monkey.dmi', "icon_state"="[wear_mask.icon_state]")
		if(MA.rgb)
			mask_l.ColorTone("[MA.rgb]")
			mask_s.ColorTone("[MA.rgb]")

		var/image/lying		= image("icon" = mask_l, "icon_state" = "[wear_mask.icon_state]2", "layer" = -M_MASK_LAYER)
		var/image/standing	= image("icon" = mask_s, "icon_state" = "[wear_mask.icon_state]", "layer" = -M_MASK_LAYER)
