...
/mob/dead/observer/verb/mobspawn()
	set category = "Ghost"
	set name = "Reincarnate" // "Haunt"
	set desc = "Respawn using runes."

	if(istype(usr, /mob/dead/observer))
		var/list/L = list()
		for(var/obj/effect/mobspawn/R in world)
			L+=R

		var/obj/effect/mobspawn/Z = input("Choose your destiny.") as null|anything in L
		if(Z)
			var/turf/T = get_turf(Z)
			usr.loc = T
			Z.spawnthatassholealreadyplease()
		else
			return
		//		var/obj/effect/mobspawn/target = R[input]
