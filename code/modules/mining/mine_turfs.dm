...
/turf/simulated/mineral/random
	...
	var/mineralSpawnChanceList = list("Uranium" = 5, "Iron" = 50, "Diamond" = 1, "Gold" = 5, "Silver" = 5, "Plasma" = 25/*, "Adamantine" =5*/, "Cave" = 5)//Currently, Adamantine won't spawn as it has no uses. -Durandan
...
/turf/simulated/floor/plating/airless/asteroid/cave/proc/make_tunnel(var/dir)
	...
	for(var/i = 0; i < length; i++)
		...
		if(istype(tunnel))
			// Chance to have forks in our tunnel; otherwise dig our tunnel.
			if(i > 3 && prob(20))
				new src.type(tunnel, rand(10, 15), 0, dir)
			else
				SpawnFloor(tunnel)
		else if(!src.blocks_air) // We hit space/normal/wall, stop our tunnel.
			break
...
/turf/simulated/floor/plating/airless/asteroid/cave/proc/SpawnFloor(var/turf/T)
	...
	if(prob(5))
		new/obj/effect/asteroidloot(T)

/* this is outdated
	var/mobSpawnChanceList = list("Hivebot" = 40, "Hivebotranged" = 35, "Hivebotrangedrapid" = 20, "Hivebotstrong" = 5, "Bear" = 5, "Russian" = 1, "Alien" = 1)//Currently, Adamantine won't spawn as it has no uses. -Durandan
	var/mobchance = 0 //means 5% chance of this plot changing to a mob

	..()
	if (prob(mobchance))
		var/mobName = pickweight(mobSpawnChanceList) //temp mob name
...
//			var/mob/living/simple_animal/hostile/M
			switch(mobName)
				...
				if("Hivebotrangedrapid")
					new/mob/living/simple_animal/hostile/hivebot/rapid(T)
				if("Hivebotstrong")
					new/mob/living/simple_animal/hostile/hivebot/strong(T)
				if("Bear")
					new/mob/living/simple_animal/hostile/bear(T)
				if("Russian")
					new/mob/living/simple_animal/hostile/russian(T)
				if("Alien")
					new/mob/living/simple_animal/hostile/alien(T) */
