...
//					usr << "\red Unfortunatly there were no candidates available"
		...
		switch(href_list["simplemake"])
			...
			if("constructarmoured")	M.change_mob_type( /mob/living/simple_animal/hostile/construct/armoured , null, null, delmob )
...
/*				if("shockwave")
			...
			if("syndies")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","SN")
				message_admins("[key_name_admin(usr)] has teleported syndies.", 1)
				E = new /datum/round_event/syndies()
			if("nanotrasen")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","SN")
				message_admins("[key_name_admin(usr)] has spawned NT security.", 1)
				E = new /datum/round_event/nanotrasen()
			if("alienspawn")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","SN")
				message_admins("[key_name_admin(usr)] has spawned alien mobs.", 1)
				E = new /datum/round_event/alienspawn()
			if("carp")
