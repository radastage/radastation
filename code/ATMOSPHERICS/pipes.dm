...
*/
		...
		dismantled
			...
			icon_state = "tank-dismantle"
			volume = 0
			anchored = 0

			attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
				if(istype(W, /obj/item/weapon/wrench))
					user << "\blue You deconstructed the tank."
					playsound(src.loc, 'sound/items/Deconstruct.ogg', 75, 1, -3)
					new/obj/item/stack/sheet/metal{amount=5}(src.loc)
					del(src)
		...
		attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
			...
			if(istype(W, /obj/item/weapon/wrench))
				user << "\blue You begin to deconstruct the [name]."
				playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
				if (do_after(user, 100))
					new/obj/machinery/atmospherics/pipe/tank/dismantled(src.loc)
					new/obj/item/stack/sheet/plasteel{amount=5}(src.loc)
					del(src)
