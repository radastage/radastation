...
/*
	New(turf/T)
		//if(!istype(T))
		//	CRASH("The first argument to the light object's constructor must be the atom that is the light source. Expected turf, received '[T]' instead.")
		..()
		owner = T
		__x = owner.x
		__y = owner.y
	...
	New(mob/M)
		//if(!istype(M))
		//	CRASH("The first argument to the light object's constructor must be the atom that is the light source. Expected mob, received '[M]' instead.")
		..()
		owner = M
		__x = owner.x
		__y = owner.y
...
area
	var/lighting_use_dynamic = 0	//Turn this flag off to prevent sd_DynamicAreaLighting from affecting this area
