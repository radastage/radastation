...
proc/random_name_lizard(gender, attempts_to_find_unique_name=10)
	for(var/i=1, i<=attempts_to_find_unique_name, i++)
		if(gender==FEMALE)	. = capitalize(pick(lizard_female)) + "-" + capitalize(pick(lizard_female))
		else				. = capitalize(pick(lizard_male)) + "-" + capitalize(pick(lizard_male))

		if(i != attempts_to_find_unique_name && !findname(.))
			break

proc/random_name_skrell()
	var/sounds = rand(2,4)
	var/sounds2 = rand(2,4)
	var/syllables = list("qr","qrr","xuq","qil","quum","xuqm","vol","xrim","zaoo","qu-uu","qix","qoo","zix","*","!")
	var/i = 0
	var/l = 0
	var/newname = ""
	var/newname2 = ""
	...
	while(l<=sounds2)
		l++
		newname2 += pick(syllables)

	. = capitalize((newname) + " " + capitalize(newname2))
