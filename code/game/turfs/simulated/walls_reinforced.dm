...
/turf/simulated/wall/r_wall/examine()
	..()
	switch(d_state)
		if(0)
			usr << "\blue You need to cut the outer grille!"
		if(1)
			usr << "\blue You need to remove the support lines!"
		if(2)
			usr << "\blue You need to slice through the metal cover!"
		if(3)
			usr << "\blue You need to pry off the cover!"
		if(4)
			usr << "\blue You need to loose the anchoring bolts!"
		if(5)
			usr << "\blue You need to slice through the support rods!"
		if(6)
			usr << "\blue You need to pry off the outer sheath!"
	return
