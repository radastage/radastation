...
/obj/machinery/vending/proc/build_inventory(list/productlist, hidden=0, req_coin=0)
	for(var/typepath in productlist)
		if(typepath in product_records)
			continue
...
/obj/machinery/vending/attackby(obj/item/weapon/W, mob/user)
	if(istype(W, /obj/item/weapon/card/emag))
		...
		return
	//else if(!W.type in product_records && !W.type in products)
	else
		if(W.type in typesof(product_records)  || W.type in typesof(products))
			user << "\red [src] already has [W] in their product list!"
			return
		else
			products += W.type
			user.drop_item()
			W.loc = src
			user << "\blue You add [W] to the [src]. Probably it will take a while for [W.name] to appear in product list."
			return


/obj/machinery/vending/verb/rename()
	...
	var/newname = copytext(sanitize(input(usr, "Enter new Vendomat name.", "Vendomat naming", new_station_name()) as null|text),1,MAX_NAME_LEN)
...
/obj/machinery/vending/verb/rebuild_vendomat()
	set name = "Rebuild Vendomat"
	set category = "Object"
	set src in oview(1)
	...
	src.product_records = list()
	src.hidden_records = list()
	src.coin_records = list()
