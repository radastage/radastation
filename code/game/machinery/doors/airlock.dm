...
/obj/machinery/door/airlock/verb/set_access()
	set name = "Set Access"
	set category = "Object"
	set desc = "Modify airlock access"
	set src in oview(1)

	if(usr.stat)
		return

	var/newaccess = copytext(sanitize(input(usr, "Enter custom access", "Access", "5") as null|text),1,999)
	src.req_access += newaccess