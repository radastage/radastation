	...
	if ((istype(W, /obj/item/weapon/tank) && !( src.destroyed )))
		...
		if(connected_port)
			...
			if(possible_port)
				...
				user << "\blue You deconstruct the [name]."
		...
		if(!destroyed)
			user << "\blue You should damage it more."
			return
		else
			user << "\red You pry some metal off [name]!"
			new /obj/item/stack/sheet/metal(src.loc)
			playsound(src.loc, 'sound/items/Crowbar.ogg', 75, 1, -3)
			if(prob(10))
				del(src)
			return
