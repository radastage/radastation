...
/obj/item/weapon/legcuffs/beartrap/HasEntered(AM as mob|obj)
	if(armed)
		...
		if(isanimal(AM) && !istype(AM, /mob/living/simple_animal/parrot) && !istype(AM, /mob/living/simple_animal/hostile/construct) && !istype(AM, /mob/living/simple_animal/shade) && !istype(AM, /mob/living/simple_animal/hostile/viscerator))
			armed = 0
...
/obj/item/weapon/ectoplasm/attack_self(mob/user)
	var/item_select = input("Crafting:", "Craft", 0) as null|anything in list("soap", "candle", "lipstick", "crayon")
	if (!item_select) return

	switch(item_select)
		if("soap")
			new /obj/item/weapon/soap(user.loc)
			user << "\blue After heating [src] in your hands, sticky substance forms a bar of soapy material."
			del(src)
		if("candle")
			new /obj/item/candle(user.loc)
			user << "\blue You twirl [src] in your hands for a while, forming a waxy stick."
			del(src)
		if("lipstick")
			new /obj/item/weapon/lipstick/random(user.loc)
			user << "\blue You suddenly realize that [src] has a nice, smooth color..."
			del(src)
		if("crayon")
			new /obj/item/toy/crayon/rainbow(user.loc)
			user << "\blue [src] looks like it could be used for writing!"
			del(src)
